...
export default function Analysis() {
  const { currentAnalysis, saveWord, savedWords, apiKey } = useApp()
  ...
  return (
    <div className="animate-fade-in max-w-2xl mx-auto space-y-6">
      {/* Lite engine banner */}
      {a.engine === 'lite' && (
        <div className="glass-card p-3 border-amber-dim/30 rounded-xl flex items-center gap-3">
          <span className="text-lg shrink-0">📖</span>
          <div className="flex-1 min-w-0 text-xs text-parchment-muted">
            Generated by WordWise's local <span className="text-amber-glow font-medium">Lite engine</span> —
            built from real etymology and curated wisdom, without a live AI call.
            {!apiKey && (
              <>
                {' '}<Link to="/settings" className="text-amber-glow hover:underline">Add an API key</Link> for deeper, personalized analysis.
              </>
            )}
          </div>
        </div>
